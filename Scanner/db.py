...
INFORMATION_DATA = [
    ...
    ("Vendor Mapping", "Maps each currently known MAC address of other devices in the network, to a vendor or manufacturer, using a website (like an API): `hwaddress.com`. An organizationally unique identifier (OUI) is a 24-bit number that uniquely identifies a vendor, manufacturer, or other organization, bought from IEEE; can be found in the MAC address in the first 3 octets. If a new MAC address is found, run the analysis again.", 5, "The manufacturer's name for each network card.", 97, 90, 0, True)
]
