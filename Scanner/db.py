...
INFORMATION_DATA = [
    ...
    ("Device Profile", "Summarise all the information about this device, from all scans and analyses, using the NetworkStorage and SpecialInformation data structures. Note: the certainty varies and depends on each scan and analysis; the certainty listed refers to retrieval from the databases.", 3, "The data are already saved, you're just viewing them.", 100, 100, 0, True),
    ...
    ("TCP Ports", "Transmission Control Protocol (TCP) Half-Open Port Scan, also known as TCP SYN Stealth Scan. A usual TCP 3-way handshake is (SYN) - (SYN+ACK) - (ACK); this scan sends a SYN packet, and waits for a SYN+ACK packet, but never completes the handshake. It asks for an **IP address** to direct the scan at, an amount of **repeats** (if it's found open once, that's enough), and a **port range** (by default from 0 to 1024). <IP dst=specific_ip> <TCP SYN>", 33, "Open TCP ports on a device", 90, 80, 1, True),
    ...
    ("Trace Route", "Using ICMP and IP:ttl, find the route to a distant device. <IP ttl=1,2,3,4...> <ICMP ping request> <~> <IP> <ICMP type=3 (Destination Unreachable) or type=0 (Echo Reply)>", 0.1, "IP addresses of all devices in the route", 0, 0, 1, True),
    ("Reveal Myself", "This is one side of the Device Discovery ability of this software. The other side, the listener, is active automatically. Allows other devices that are running this software to discover that I'm running it too.\nThis doesn't let me discover others, if they want to remain hidden, it just reveals myself.\nDiscovering other devices is automatically started when the software starts, and it doesn't damage your hiddenness.\nImplementation-wise, this sends out broadcast UDP packets, continuously. <Ether> <IP dst=broadcast> <UDP> <Raw: chosen name>", 0, "You gain nothing on this device, but you gain knowledge about 'Who's running this software?' on other devices.", 95, 0, 1, False),
    ...
    # ("Recommended Scan Algorithm", "See a visualisation explaining the Recommend Scan feature, and the learnt behaviour of the current user (as graph weights). Usually doesn't work due to the non-threadable nature of matplotlib", 1, "Understanding the Algorithm", 100, 100, 0, True)
]
