...
INFORMATION_DATA = [
    ...
    ("TCP Ports", "Transmission Control Protocol (TCP) Half-Open Port Scan. A usual TCP 3-way handshake is (SYN) - (SYN+ACK) - (ACK); this scan sends a SYN packet, and waits for a SYN+ACK packet, but never completes the handshake. It asks for an **IP address** to direct the scan at, an amount of **repeats** (if it's found open once, that's enough), and a **port range** (by default from 0 to 1024). <IP dst=specific_ip> <TCP SYN>", 33, "Open TCP ports on a device", 90, 80, 1, True),
